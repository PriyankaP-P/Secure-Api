...
function tfaLogin(req, res) {
  const { userToken } = req.params;
  let verified;

  // console.log(req.sendingToken);
  knex
    .table("users")
    .where({ id: req.sendingToken.id })
    .select("key_tfa")
    .then(theKey => {
      console.log(theKey[0].key_tfa);
      if (!theKey) {
        errors.notheKey = "Not found";
        return res.status(404).json(errors);
      }

      verified = speakeasy.totp({
        secret: theKey[0].key_tfa,
        encoding: "base32",
        token: userToken
      });
